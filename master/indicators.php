...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if ($action === 'create') {
            $stmt = $pdo->prepare("INSERT INTO tbl_indicators (indicator_description) VALUES (:desc)");
            $stmt->execute([':desc' => trim($_POST['indicator_description'] ?? '')]);
            echo json_encode(['status' => 'success', 'message' => 'Indicator added.']);
        } elseif ($action === 'update') {
            $stmt = $pdo->prepare("UPDATE tbl_indicators SET indicator_description=:desc WHERE id=:id");
            $stmt->execute([':id' => (int)$_POST['id'], ':desc' => trim($_POST['indicator_description'] ?? '')]);
            echo json_encode(['status' => 'success', 'message' => 'Indicator updated.']);
        } elseif ($action === 'delete') {
            $stmt = $pdo->prepare("DELETE FROM tbl_indicators WHERE id=:id");
            $stmt->execute([':id' => (int)$_POST['id']]);
            echo json_encode(['status' => 'success', 'message' => 'Indicator deleted.']);
        } else {
            throw new InvalidArgumentException('Invalid action.');
        }
    } catch (PDOException $e) {
        http_response_code(500);
        $msg = str_contains($e->getMessage(), 'foreign key') ? 'Cannot delete: this record is in use.' : 'A database error occurred.';
...
$pageTitle  = 'Performance Indicators';
$activeMenu = 'indicators';
try { $rows = getConnection()->query("SELECT * FROM tbl_indicators ORDER BY id")->fetchAll(); } catch (PDOException $e) { $rows = []; }

require_once __DIR__ . '/../includes/header.php';
?>

<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
    <p class="text-sm text-gray-500">Manage performance indicators used in budget proposals.</p>
    <button onclick="openModal()" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-lg bg-brand-600 text-white text-sm font-semibold hover:bg-brand-700 transition shadow-md whitespace-nowrap">
        <i class="fa-solid fa-plus text-xs"></i> Add Indicator
    </button>
</div>
        ...
        <table id="dataTable" class="w-full text-left">
            <thead>
                <tr class="border-b border-gray-200">
                    ...
                    <th class="py-3 px-2">Indicator Description</th>
            ...
            <tbody>
                ...
                <tr class="border-b border-gray-50">
                    ...
                    <td class="py-3 px-2 text-gray-800"><?= e($r['indicator_description']) ?></td>
...
<div id="modal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 hidden">
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-lg mx-4 overflow-hidden">
        <div class="bg-brand-50 px-6 py-4 border-b flex items-center justify-between">
            <h3 class="text-base font-semibold text-brand-800" id="modalTitle">Add Indicator</h3>
        ...
        <form id="modalForm" class="p-6 space-y-4">
            ...
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1">Indicator Description <span class="text-red-500">*</span></label>
                <textarea name="indicator_description" id="fDesc" rows="3" required class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 transition resize-none" placeholder="e.g. Number of patients served"></textarea>
            </div>
...
<script>
$(function(){ if($('#dataTable tbody tr').length) $('#dataTable').DataTable({pageLength:15, order:[[0,'asc']], columnDefs:[{orderable:false,targets:[3]}], language:{search:'',searchPlaceholder:'Search…'}}); });

function openModal(data) {
    document.getElementById('modalTitle').textContent = data ? 'Edit Indicator' : 'Add Indicator';
    document.getElementById('fId').value   = data ? data.id : '';
    document.getElementById('fDesc').value = data ? data.indicator_description : '';
...
function deleteRow(id) {
    Swal.fire({title:'Delete this indicator?',text:'This cannot be undone.',icon:'warning',showCancelButton:true,confirmButtonText:'Delete',confirmButtonColor:'#ef4444',cancelButtonColor:'#6b7280',reverseButtons:true})
    .then(r => {
        if (!r.isConfirmed) return;
        const fd = new FormData(); fd.append('action','delete'); fd.append('id',id);
        fetch(window.location.href,{method:'POST',body:fd}).then(r=>r.json()).then(d=>{
            if(d.status==='success'){Swal.fire({icon:'success',title:'Deleted!',text:d.message,confirmButtonColor:'#0b4d26',timer:1500,showConfirmButton:false}).then(()=>location.reload());}
