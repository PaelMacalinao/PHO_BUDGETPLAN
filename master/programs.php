<?php
/**
 * Master Data CRUD — Programs / Units
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if ($action === 'create') {
            $stmt = $pdo->prepare("INSERT INTO tbl_programs_units (program_name) VALUES (:name)");
            $stmt->execute([':name' => trim($_POST['program_name'] ?? '')]);
            echo json_encode(['status' => 'success', 'message' => 'Program added.']);
        } elseif ($action === 'update') {
            $stmt = $pdo->prepare("UPDATE tbl_programs_units SET program_name=:name WHERE id=:id");
            $stmt->execute([':id' => (int)$_POST['id'], ':name' => trim($_POST['program_name'] ?? '')]);
            echo json_encode(['status' => 'success', 'message' => 'Program updated.']);
        } elseif ($action === 'delete') {
            $stmt = $pdo->prepare("DELETE FROM tbl_programs_units WHERE id=:id");
        ...
        $msg = str_contains($e->getMessage(), 'Duplicate entry') ? 'This program already exists.' :
...
$pageTitle  = 'Programs (PPA)';
$activeMenu = 'programs';
try { $rows = getConnection()->query("SELECT * FROM tbl_programs_units ORDER BY program_name")->fetchAll(); } catch (PDOException $e) { $rows = []; }

require_once __DIR__ . '/../includes/header.php';
?>

<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
    <p class="text-sm text-gray-500">Manage programs / projects / activities (PPA) used in budget proposals.</p>
    <button onclick="openModal()" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-lg bg-brand-600 text-white text-sm font-semibold hover:bg-brand-700 transition shadow-md whitespace-nowrap">
        <i class="fa-solid fa-plus text-xs"></i> Add Program
    </button>
</div>
        ...
        <table id="dataTable" class="w-full text-left">
            <thead>
                <tr class="border-b border-gray-200">
                    ...
                    <th class="py-3 px-2">Program Name</th>
            ...
            <tbody>
                ...
                <tr class="border-b border-gray-50">
                    ...
                    <td class="py-3 px-2 font-semibold text-gray-800"><?= e($r['program_name']) ?></td>
...
<div id="modal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 hidden">
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-md mx-4 overflow-hidden">
        <div class="bg-brand-50 px-6 py-4 border-b flex items-center justify-between">
            <h3 class="text-base font-semibold text-brand-800" id="modalTitle">Add Program</h3>
        ...
        <form id="modalForm" class="p-6 space-y-4">
            ...
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1">Program Name <span class="text-red-500">*</span></label>
                <input type="text" name="program_name" id="fName" required class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 transition" placeholder="e.g. PHO CLINIC">
            </div>
...
function openModal(data) {
    document.getElementById('modalTitle').textContent = data ? 'Edit Program' : 'Add Program';
    document.getElementById('fId').value   = data ? data.id : '';
    document.getElementById('fName').value = data ? data.program_name : '';
...
function deleteRow(id) {
    Swal.fire({title:'Delete this program?',text:'This cannot be undone.',icon:'warning',showCancelButton:true,confirmButtonText:'Delete',confirmButtonColor:'#ef4444',cancelButtonColor:'#6b7280',reverseButtons:true})
    .then(r => {
