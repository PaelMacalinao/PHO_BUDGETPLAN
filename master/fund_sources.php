...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if ($action === 'create') {
            $stmt = $pdo->prepare("INSERT INTO tbl_fund_sources (fund_name) VALUES (:name)");
            $stmt->execute([':name' => trim($_POST['fund_name'] ?? '')]);
            echo json_encode(['status' => 'success', 'message' => 'Fund source added.']);
        } elseif ($action === 'update') {
            $stmt = $pdo->prepare("UPDATE tbl_fund_sources SET fund_name=:name WHERE id=:id");
            $stmt->execute([':id' => (int)$_POST['id'], ':name' => trim($_POST['fund_name'] ?? '')]);
            echo json_encode(['status' => 'success', 'message' => 'Fund source updated.']);
        } elseif ($action === 'delete') {
            $stmt = $pdo->prepare("DELETE FROM tbl_fund_sources WHERE id=:id");
            $stmt->execute([':id' => (int)$_POST['id']]);
            echo json_encode(['status' => 'success', 'message' => 'Fund source deleted.']);
        } else {
            throw new InvalidArgumentException('Invalid action.');
        }
    } catch (PDOException $e) {
        http_response_code(500);
        $msg = str_contains($e->getMessage(), 'Duplicate entry') ? 'This fund source already exists.' :
               (str_contains($e->getMessage(), 'foreign key') ? 'Cannot delete: this record is in use.' : 'A database error occurred.');
...
$pageTitle  = 'Fund Sources';
$activeMenu = 'fund_sources';
try { $rows = getConnection()->query("SELECT * FROM tbl_fund_sources ORDER BY fund_name")->fetchAll(); } catch (PDOException $e) { $rows = []; }

require_once __DIR__ . '/../includes/header.php';
?>

<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
    <p class="text-sm text-gray-500">Manage fund sources used in budget proposals.</p>
    <button onclick="openModal()" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-lg bg-brand-600 text-white text-sm font-semibold hover:bg-brand-700 transition shadow-md whitespace-nowrap">
        <i class="fa-solid fa-plus text-xs"></i> Add Fund Source
    </button>
</div>
        ...
        <table id="dataTable" class="w-full text-left">
            <thead>
                <tr class="border-b border-gray-200">
                    ...
                    <th class="py-3 px-2">Fund Name</th>
            ...
            <tbody>
                ...
                <tr class="border-b border-gray-50">
                    ...
                    <td class="py-3 px-2 font-semibold text-gray-800"><?= e($r['fund_name']) ?></td>
...
<div id="modal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 hidden">
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-md mx-4 overflow-hidden">
        <div class="bg-brand-50 px-6 py-4 border-b flex items-center justify-between">
            <h3 class="text-base font-semibold text-brand-800" id="modalTitle">Add Fund Source</h3>
        ...
        <form id="modalForm" class="p-6 space-y-4">
            ...
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1">Fund Name <span class="text-red-500">*</span></label>
                <input type="text" name="fund_name" id="fName" required class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 transition" placeholder="e.g. General Fund">
            </div>
...
function openModal(data) {
    document.getElementById('modalTitle').textContent = data ? 'Edit Fund Source' : 'Add Fund Source';
    document.getElementById('fId').value   = data ? data.id : '';
    document.getElementById('fName').value = data ? data.fund_name : '';
...
function deleteRow(id) {
    Swal.fire({title:'Delete this fund source?',text:'This cannot be undone.',icon:'warning',showCancelButton:true,confirmButtonText:'Delete',confirmButtonColor:'#ef4444',cancelButtonColor:'#6b7280',reverseButtons:true})
    .then(r => {
