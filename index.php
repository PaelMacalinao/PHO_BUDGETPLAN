...
try {
    ...
    // 1) Grand totals per fund source
    ...
    // 2) General Fund → aggregate by Expense Class
    $gfByClass = $pdo->query("
    ...
    // 3) Special Project → aggregate by Unit/Project, then Expense Class
    $spByUnit = $pdo->query("
        ...
        ORDER BY un.unit_name, FIELD(ac.expense_class, 'PS', 'MOOE', 'CO')
    ")->fetchAll();
...
$spProjects = [];
foreach ($spByUnit as $r) {
    $p = $r['project_name'];
    if (!isset($spProjects[$p])) {
        $spProjects[$p] = ['total' => 0, 'count' => 0, 'classes' => []];
    }
    $spProjects[$p]['classes'][$r['expense_class']] = (float)$r['class_total'];
    $spProjects[$p]['total'] += (float)$r['class_total'];
    $spProjects[$p]['count'] += (int)$r['proposal_count'];
}
...
<?php if ($totalProposals === 0 && !isset($dbError)): ?>
<!-- ═══ EMPTY STATE ═══ -->
...
<!-- ═══════════════════════════════════════════════════
     LEVEL 2 — SPECIAL PROJECT: By Project, then Class
     ═══════════════════════════════════════════════════ -->
<div id="view-sp" class="fund-view" style="display:none">

    <h3 class="text-sm font-bold text-gray-400 uppercase tracking-wider mb-4">
        <i class="fa-solid fa-diagram-project mr-1.5 text-amber-500"></i> Special Project &mdash; Breakdown by Project
    </h3>
        ...
        <div class="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden exp-card">
                ...
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                    <div class="flex items-center gap-3 min-w-0">
                        ...
                        <div class="min-w-0">
                            ...
                            <span class="block text-[11px] text-gray-400"><?= number_format($projData['count']) ?> proposal<?= $projData['count'] !== 1 ? 's' : '' ?> &middot; <?= $projPct ?>% of fund</span>
                        </div>
                    </div>
            ...
            <div class="p-5 grid grid-cols-1 sm:grid-cols-3 gap-3">
                ...
                <div class="flex items-center gap-3 px-4 py-3 rounded-lg <?= $ci['bg'] ?> border-l-4 <?= $ci['border'] ?>">
                    ...
                    <div class="min-w-0">
                        <span class="block text-xs font-semibold text-gray-500"><?= $cls ?> <span class="text-gray-400 font-normal">&middot; <?= $clsPct ?>%</span></span>
...
(function() {
    ...
    window.switchFund = function(fund) {
        ...
        Object.keys(tabs).forEach(function(key) {
            tabs[key].classList.toggle('active', key === fund);
        });
